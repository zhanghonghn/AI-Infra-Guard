...
class TestProbeResult(unittest.TestCase):
    ...
    def _make_probe(self, **kwargs):
        defaults = dict(
            ...
            total_attempts=10,
            total_failures=3,
            examples=[{"prompt": "p", "response": "r", "passed": False}],
        )
        defaults.update(kwargs)
        return ProbeResult(**defaults)
    ...
    def test_to_dict_all_fields(self):
        p = self._make_probe()
        d = p.to_dict()
        for key in ("probe_id", "passed", "total_attempts", "total_failures", "examples"):
            self.assertIn(key, d, f"字段 {key} 应存在于 to_dict() 输出")
...
class TestAdapterResult(unittest.TestCase):
    ...
    def _make_result(self):
        ...
        return AdapterResult(
            ...
            status="completed",
            probe_results=[probe],
            metadata={"intensity": "fast", "model": "gpt-4o", "provider": "openai",
                       "start_time": "2026-01-01T00:00:00Z", "end_time": "2026-01-01T00:15:00Z",
                       "total_probes": 1, "scan_duration_seconds": 900},
            summary={"total_attempts": 5, "total_failures": 2, "pass_rate": 0.6},
        )

    def test_to_dict_top_level_keys(self):
        d = self._make_result().to_dict()
        for key in ("scan_id", "status", "probe_results", "metadata", "summary"):
            self.assertIn(key, d)
    ...
    def test_to_dict_metadata_keys(self):
        d = self._make_result().to_dict()
        for key in ("intensity", "model", "provider", "start_time", "end_time",
                    "total_probes", "scan_duration_seconds"):
            self.assertIn(key, d["metadata"], f"metadata 缺少字段: {key}")
...
class TestGarakRunnerMockMode(unittest.TestCase):
    ...
    def test_mock_run_pass_rate_range(self):
        ...
        runner = self._make_runner()
        result = runner.run()
        rate = result.summary.get("pass_rate", -1)
        self.assertGreaterEqual(rate, 0.0)
        self.assertLessEqual(rate, 1.0)
...
class TestContractWithFixture(unittest.TestCase):
    ...
    def test_each_probe_result_has_required_fields(self):
        for probe in self.fixture["probe_results"]:
            for field in ("probe_id", "passed", "total_attempts", "total_failures", "examples"):
                self.assertIn(field, probe, f"probe_result 缺少字段: {field}")
    ...
    def test_metadata_has_required_fields(self):
        meta = self.fixture["metadata"]
        for field in ("model", "provider", "intensity", "total_probes", "scan_duration_seconds"):
            self.assertIn(field, meta, f"metadata 缺少字段: {field}")

    def test_pass_rate_in_valid_range(self):
        rate = self.fixture["summary"]["pass_rate"]
        self.assertGreaterEqual(rate, 0.0)
        self.assertLessEqual(rate, 1.0)

    def test_total_attempts_and_failures_consistent(self):
        summary = self.fixture["summary"]
        self.assertGreaterEqual(summary["total_attempts"], summary["total_failures"])
